...
int main () {
    ...
    for (int i=0;i<index;i++) {
        if (logbook[i].data[0] == 'G') {
            ...
            if (regiment[currentguard.id].id == 0) {regiment[currentguard.id].id = currentguard.id;}
            ...
            for (int j=fallsasleep;j<wakesup;j++) {
                ...
                if (currentguard.sleeptime > sleepy.sleeptime) {
                    sleepy = currentguard;
                }
            }
    ...
    int max = 0;
    for (int i=0;i<60;i++) {
        if (sleepy.minutes[i] > max) {
            maxminute = i;
            max = sleepy.minutes[i];
        }
    }
    printf("Final Result: %d", sleepy.id*maxminute);
    free(logbook);
    fclose(input);
    return 0;
}
